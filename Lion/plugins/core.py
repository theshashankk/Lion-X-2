...
DELETE_TIMEOUT = 5
thumb_image_path = "./resources/Lion.jpeg"
        ...
        try:
            downloaded_file_name = (
                await event.client.download_media(  # pylint:disable=E0602
                    ...
                    "Lion/plugins/",  # pylint:disable=E0602
                )
            )
...
async def load(event):
    ...
    try:
        ...
        await event.edit(f"Lion has successfully loaded {shortname}")
    except Exception as e:
        await event.edit(
            f"Lion could not load {shortname} because of the following error.\n{str(e)}"
        )
...
async def install(event):
    ...
    for ixo in total_doxx:
        ...
        downloaded_file_name = await event.client.download_media(
            await event.client.get_messages(event.chat_id, ids=mxo), "Lion/plugins/"
        )
        if "(" not in downloaded_file_name:
            path1 = Path(downloaded_file_name)
            shortname = path1.stem
